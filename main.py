...
SYSTEM_PROMPT = (
    ...
    "(читач не повинен переходити за посиланням щоб зрозуміти суть).\n"
    "Відповідай ТІЛЬКИ валідним JSON без зайвого тексту:\n"
    '{"national":[{"title":"...","summary":"...","link":"..."}],"local":[...]}'
)

def ai_summarize(items: list) -> dict:
    resp = requests.post(
...
# ── Message builder ──────────────────────────────────────────────────────────
def build_message(now: datetime, weather: str, currencies: str, digest: dict) -> str:
...
@app.route('/')
def index():
    return "Маньківка 8:00 | running ✓", 200
...
# ── Scheduler ────────────────────────────────────────────────────────────────
_sched = BackgroundScheduler(timezone=KYIV)
_sched.add_job(send_digest, CronTrigger(hour=8,  minute=0, timezone=KYIV))
_sched.add_job(send_digest, CronTrigger(hour=13, minute=0, timezone=KYIV))
_sched.start()
atexit.register(lambda: _sched.stop())
